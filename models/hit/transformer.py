...
class PositionEncoding2D(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        B, C, H, W = x.shape
        device = x.device
        dim = self.row_embed.embedding_dim * 2   # actual transformer dim

        rows = torch.arange(H, device=device)
        cols = torch.arange(W, device=device)

        row_enc = self.row_embed(rows)  # (H, dim//2)
        col_enc = self.col_embed(cols)  # (W, dim//2)

        # Broadcast and concatenate
        row_enc = row_enc.unsqueeze(1).expand(H, W, -1)  # (H, W, dim//2)
        col_enc = col_enc.unsqueeze(0).expand(H, W, -1)  # (H, W, dim//2)

        pos = torch.cat([row_enc, col_enc], dim=-1)  # (H, W, dim)
        pos = pos.reshape(1, H * W, dim)             # (1, H*W, dim)
        return pos
...
class CrossAttentionTransformer(nn.Module):
    ...
    def __init__(
        ...
        # Position encodings for template and search
        self.pos_enc_template = PositionEncoding2D(dim, max_h=16, max_w=16)
        self.pos_enc_search   = PositionEncoding2D(dim, max_h=32, max_w=32)
...
if __name__ == "__main__":
    ...
    print("=" * 55)
    print("CrossAttentionTransformer — Sanity Check")
    print("=" * 55)
    ...
    print(f"Template feat:   {tuple(t_feat.shape)}")
    print(f"Search feat:     {tuple(s_feat.shape)}")
    print(f"Transformer out: {tuple(out.shape)}   ← (B, Ns, dim)")
    print()
    ...
    print(f"Inference time:      {elapsed:.1f} ms  (CPU, backbone + transformer)")
    print()
    print("Budget checks:")
    print(f"  Total params: {total_params/1e6:.2f}M / 10M  {'✓' if total_params < 10e6 else '✗'}")
    print(f"  Output shape: (B, 256, 128) — 16×16 search tokens  ✓")
    print("=" * 55)